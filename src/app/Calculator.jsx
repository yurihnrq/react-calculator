...
const Calculator = (_) => {
  ...
  const addDigit = useCallback(
    (n) => {
      ...
      if (displayHandler.main.indexOf(".") >= 0 && n === ".") return;
      ...
      if (n !== "=") {
        ...
        if (
          ...
          if (n !== displayHandler.expression[lastIndex])
            displayHandler.expression =
              displayHandler.expression.substr(0, lastIndex) +
              n +
              displayHandler.expression.substr(lastIndex + 1);
        } else displayHandler.expression = displayHandler.expression + n;
        ...
        if (operators.includes(expression[expression.length - 1])) return;
  ...
  const handleKeyPress = useCallback(
    (event) => {
      ...
      if (keyCode >= 48 && keyCode <= 57 && !isNaN(key)) addDigit(key);
      else if (operatorsKeys.indexOf(key) > -1) {
        if (key === "*") addDigit("×");
        else if (key === "/") addDigit("÷");
        else addDigit(key);
      } else if (keyCode === 46) addDigit("DEL");
      else if (key === "=" || key === "Enter") addDigit("=");
    },
    [addDigit]
  );
